...
bool PhysicalCollectionMock::applyForTickRange(TRI_voc_tick_t dataMin, TRI_voc_tick_t dataMax, std::function<bool(TRI_voc_tick_t foundTick, TRI_df_marker_t const* marker)> const& callback) {
  TRI_ASSERT(false);
  return false;
}
...
std::unique_ptr<arangodb::IndexIterator> PhysicalCollectionMock::getAllIterator(arangodb::transaction::Methods* trx, arangodb::ManagedDocumentResult* mdr, bool reverse) {
  TRI_ASSERT(false);
  return nullptr;
}

std::unique_ptr<arangodb::IndexIterator> PhysicalCollectionMock::getAnyIterator(arangodb::transaction::Methods* trx, arangodb::ManagedDocumentResult* mdr) {
  TRI_ASSERT(false);
  return nullptr;
}
...
void PhysicalCollectionMock::invokeOnAllElements(std::function<bool(arangodb::DocumentIdentifierToken const&)> callback) {
  TRI_ASSERT(false);
}
...
uint64_t PhysicalCollectionMock::numberDocuments() const {
  TRI_ASSERT(false);
  return 0;
}
...
TRI_voc_rid_t PhysicalCollectionMock::revision() const {
  TRI_ASSERT(false);
  return 0;
}
...
arangodb::StorageEngine::Database* StorageEngineMock::createDatabase(TRI_voc_tick_t id, arangodb::velocypack::Slice const& args, int& status) {
  TRI_ASSERT(false);
  return nullptr;
}
...
arangodb::TransactionState* StorageEngineMock::createTransactionState(TRI_vocbase_t* vocbase) {
...
void StorageEngineMock::dropDatabase(Database*, int& status) {
  TRI_ASSERT(false);
}
...
arangodb::StorageEngine::Database* StorageEngineMock::openDatabase(arangodb::velocypack::Slice const& args, bool isUpgrade, int& status) {
  TRI_ASSERT(false);
  return nullptr;
}
...
int StorageEngineMock::writeCreateMarker(TRI_voc_tick_t id, VPackSlice const& slice) {
  TRI_ASSERT(false);
  return TRI_ERROR_INTERNAL;
}
...
int TransactionStateMock::abortTransaction(arangodb::transaction::Methods* trx) {
...
int TransactionStateMock::beginTransaction(arangodb::transaction::Hints hints) {
...
int TransactionStateMock::commitTransaction(arangodb::transaction::Methods* trx) {
